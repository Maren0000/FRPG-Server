package ProcessFiles

import (
	Consts_LuaHash "FRPGServer/Consts/LuaHash"
	Consts_Quest "FRPGServer/Consts/Quest"
	Consts_QuestItem "FRPGServer/Consts/QuestItem"
	Consts_ScanTag "FRPGServer/Consts/ScanTag"
	db_commands "FRPGServer/db/commands"
)

func EX_EV_Quiz_1_Clear(UserID string) error {
	/*err := db_commands.UpdateUserScanRemove(UserID, Consts_ScanTag.QR_EX_Quiz_1_109, 1)
	if err != nil {
		return err
	}*/
	//Not in SERVER_SCRIPT
	err := db_commands.UpdateUserScanLuaHash(UserID, Consts_ScanTag.QR_EX_Quiz_1_109, Consts_LuaHash.EX_EV_Quiz_1_Def)
	if err != nil {
		return err
	}

	//Remove GPS
	err = db_commands.UpdateUserGPSRemove(UserID, "GPS_EX_Shibuya_109", 1)
	if err != nil {
		return err
	}

	err = db_commands.CreateUserQuestItem(UserID, Consts_Quest.Quest_100_Secret_Boss, Consts_QuestItem.Quest_100_Hint_2, "on")
	if err != nil {
		return err
	}

	Q109_Cleared, err := db_commands.CheckUserQuestCleared(UserID, Consts_Quest.Quest_109_EX_Quiz1)
	if err != nil {
		return err
	}
	if !Q109_Cleared {
		err = db_commands.UpdateUserQuestClear(UserID, Consts_Quest.Quest_109_EX_Quiz1, 1)
		if err != nil {
			return err
		}
	}

	//Not in SERVER_SCRIPT. Mainly to avoid issues later on.
	err = db_commands.UpdateUserResumeLuaResume(UserID, 0)
	if err != nil {
		return err
	}

	return nil
}
